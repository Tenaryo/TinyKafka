...
TEST(SerializerTest, SerializesApiVersionsResponseValid) {
    ...
    EXPECT_EQ(bytes[3], 0x0D);  // message_size = 13
    EXPECT_EQ(bytes[8], 0x00);  // TAG_BUFFER (response header v1)
    EXPECT_EQ(bytes[11], 0x01); // compact array length = 0 (encoded as 1)
    EXPECT_EQ(bytes[16], 0x00); // TAG_BUFFER
}
...
TEST(SerializerTest, SerializesApiVersionsResponseUnsupported) {
    ...
    ASSERT_EQ(bytes.size(), 17);
    EXPECT_EQ(bytes[10], 0x23); // error_code = 35
}
...
TEST(SerializerTest, SerializesFullApiVersionsResponse) {
    ...
    EXPECT_EQ(bytes[1], 0x00);
    EXPECT_EQ(bytes[2], 0x00);
    EXPECT_EQ(bytes[3], 0x14); // message_size = 20
    ...
    EXPECT_EQ(bytes[8], 0x00); // TAG_BUFFER (response header v1)
    EXPECT_EQ(bytes[9], 0x00);
    EXPECT_EQ(bytes[10], 0x00); // error_code = 0
    EXPECT_EQ(bytes[11], 0x02); // compact array length = 1
    EXPECT_EQ(bytes[12], 0x00);
    EXPECT_EQ(bytes[13], 0x12); // api_key = 18
    EXPECT_EQ(bytes[14], 0x00);
    EXPECT_EQ(bytes[15], 0x00); // min_version = 0
    EXPECT_EQ(bytes[16], 0x00);
    EXPECT_EQ(bytes[17], 0x04); // max_version = 4
    EXPECT_EQ(bytes[18], 0x00); // TAG_BUFFER (entry)
    EXPECT_EQ(bytes[19], 0x00);
    EXPECT_EQ(bytes[20], 0x00);
    EXPECT_EQ(bytes[21], 0x04);
    EXPECT_EQ(bytes[22], 0xd2); // throttle_time_ms = 1234
    EXPECT_EQ(bytes[23], 0x00); // TAG_BUFFER
}
