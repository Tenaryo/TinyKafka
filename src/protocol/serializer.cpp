        ...
        overloaded{
            [](const ApiVersionsResponse& r) -> std::vector<std::uint8_t> {
                ...
                const size_t body_size =
                    kResponseHeaderV1Size + 2 + varint_len + r.api_keys.size() * 7 + 4 + 1;
                ...
                write_int16_be(r.error_code,
                               std::span<uint8_t, 2>{buf.data() + 4 + kResponseHeaderV1Size, 2});

                size_t offset = 4 + kResponseHeaderV1Size + 2;
